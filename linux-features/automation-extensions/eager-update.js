...
const DYNAMIC = /\.map\(([A-Za-z_$][\w$]*)=>\(\{type:`function`,\.\.\.\1,\.\.\.([A-Za-z_$][\w$]*)&&!([A-Za-z_$][\w$]*)\.has\(\1\.name\)\?\{deferLoading:!0\}:\{\}\}\)\)/u;
...
function applyAutomationUpdateEagerToolPatch(source) {
  ...
  return source.replace(DYNAMIC, (_match, tool, namespaceFlag, eagerTools) => {
    const descriptor = tool === "t" ? "codexLinuxAutomationDescriptor" : "t";
    return `.map(${tool}=>{let ${descriptor}={type:\`function\`,...${tool},...${namespaceFlag}&&!${eagerTools}.has(${tool}.name)?{deferLoading:!0}:{}};return ${tool}.name===\`automation_update\`&&delete ${descriptor}.deferLoading,${descriptor}})`;
  });
}
