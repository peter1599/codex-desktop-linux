...
const currentAppInfoSource = [
  ...
  "function hS(e){if(e==null)return null;let t=l.app.isPackaged?(0,p.join)(process.resourcesPath,e):null,n=t!=null&&(0,_.existsSync)(t)?t:(0,p.join)(l.app.getAppPath(),`src`,`icons`,e),r=l.nativeImage.createFromPath(n);return r.isEmpty()?null:r.resize({width:128,height:128,quality:`best`}).toDataURL()}",
].join("");
...
test("main patch rejects drift at every official-package insertion point byte-identically", () => {
  ...
  const patchedPoints = [
    ...
    "I=()=>{if(!v&&process.platform!==`linux`)return;",
    "if(v||process.platform===`linux`){I();let e=()=>",
