...
const STAGED_FEATURE_MANIFEST_RELATIVE_PATH =
	".codex-linux/linux-features-staged.json";
...
function readJsonFile(filePath, label, options = {}) {
	try {
		...
		console.warn(
			`WARN: Could not read ${label} at ${filePath}: ${error.message}`,
		);
		return null;
	}
}
...
function readLinuxFeaturesConfig(options = {}) {
	...
	if (!fs.existsSync(configPath)) {
		if (strict) {
			throw new Error(
				`Could not read Linux features config at ${configPath}: file does not exist`,
			);
		}
	...
	if (config == null) {
		if (strict) {
			throw new Error(
				`Linux features config ${configPath} must be a JSON object`,
			);
		}
		return { config: null, configPath };
	}
	if (typeof config !== "object" || Array.isArray(config)) {
		...
		console.warn(
			`WARN: Linux features config ${configPath} must be a JSON object`,
		);
		return { config: null, configPath };
	}
...
function normalizeEnabledFeatureIds(value, sourcePath, options = {}) {
	if (!Array.isArray(value)) {
		if (options.strict === true) {
			throw new Error(
				`Linux features config ${sourcePath} must contain an enabled array`,
			);
		}
		console.warn(
			`WARN: Linux features config ${sourcePath} must contain an enabled array`,
		);
		return [];
	}
	...
	for (const item of value) {
		if (typeof item !== "string" || !FEATURE_ID_PATTERN.test(item)) {
			if (options.strict === true) {
				throw new Error(
					`Invalid Linux feature id in ${sourcePath}: ${String(item)}`,
				);
			}
			console.warn(
				`WARN: Invalid Linux feature id in ${sourcePath}: ${String(item)}`,
			);
			continue;
		}
...
function enabledFeatureIdsFromBuildInfo(appDir) {
	...
	try {
		...
		throw new Error(
			`Could not read packaged app build info at ${buildInfoPath}: ${error.message}`,
		);
	}
	if (
		buildInfo == null ||
		...
		throw new Error(
			`Packaged app build info at ${buildInfoPath} must be a JSON object`,
		);
	}
	const enabled = buildInfo.linuxFeatures?.enabled;
	if (!Array.isArray(enabled)) {
		throw new Error(
			`Packaged app build info at ${buildInfoPath} must contain linuxFeatures.enabled`,
		);
	}
	...
	for (const rawId of enabled) {
		...
		if (seen.has(id)) {
			throw new Error(
				`Duplicate Linux feature id in ${buildInfoPath}: ${rawId}`,
			);
		}
		seen.add(id);
		ids.push(id);
...
function normalizeLinuxFeatureSettings(value, sourcePath) {
	...
	if (typeof value !== "object" || Array.isArray(value)) {
		console.warn(
			`WARN: Linux features config ${sourcePath} settings must be an object`,
		);
		return {};
	}
	...
	for (const [rawId, rawSettings] of Object.entries(value)) {
		if (typeof rawId !== "string" || !FEATURE_ID_PATTERN.test(rawId)) {
			console.warn(
				`WARN: Invalid Linux feature settings id in ${sourcePath}: ${String(rawId)}`,
			);
			continue;
		}
		...
		if (
			...
			console.warn(
				`WARN: Linux feature '${rawId}' settings in ${sourcePath} must be an object`,
			);
			continue;
		}
...
function featureManifestCandidates(featuresRoot) {
	...
	for (const name of fs.readdirSync(featuresRoot).sort()) {
		...
		if (isDirectory(dir) && fs.existsSync(path.join(dir, "feature.json"))) {
			candidates.push({
				dir,
				manifestPath: path.join(dir, "feature.json"),
				origin: "repo",
			});
		}
	}
...
function normalizeLinuxFeatureManifest(featuresRoot, candidate) {
	...
	if (
		...
		throw new Error(
			`Linux feature manifest ${candidate.manifestPath} must be a JSON object`,
		);
	}
...
function linuxFeatureManifestMap(options = {}) {
	return new Map(
		discoverLinuxFeatureManifests(options).map((feature) => [
			feature.id,
			feature,
		]),
	);
...
function loadLinuxFeatureManifest(featuresRoot, id, options = {}) {
	...
	if (feature == null) {
		console.warn(
			`WARN: Enabled Linux feature '${id}' does not have feature.json`,
		);
		return null;
	}
	return feature;
	...
	for (const feature of features) {
		for (const required of feature.manifest.requires) {
			if (!enabled.has(required)) {
				throw new Error(
					`Linux feature '${feature.id}' requires '${required}' to be enabled`,
				);
			}
		}
		for (const conflict of feature.manifest.conflicts) {
			if (enabled.has(conflict)) {
				throw new Error(
					`Linux feature '${feature.id}' conflicts with '${conflict}'`,
				);
			}
		}
	}
...
function resolveFeatureRelativePath(
	feature,
	relativePath,
	label,
	{ mustExist = true } = {},
	...
	if (
		...
		relativePathParts(relativePath).includes("..")
	) {
		throw new Error(
			`Linux feature '${feature.id}' ${label} must stay inside the feature directory`,
		);
	}
	...
	if (mustExist && !fs.existsSync(resolved)) {
		throw new Error(
			`Linux feature '${feature.id}' ${label} not found: ${resolved}`,
		);
	}
	return resolved;
}
...
function wrapFeaturePatchDescriptor(
	feature,
	descriptor,
	sourcePath,
	index,
	...
	if (descriptor == null || typeof descriptor !== "object") {
		console.warn(
			`WARN: Linux feature '${feature.id}' patch descriptor ${index + 1} must be an object`,
		);
		return null;
	}
	if (typeof descriptor.apply !== "function") {
		console.warn(
			`WARN: Linux feature '${feature.id}' patch descriptor ${index + 1} must export apply`,
		);
		return null;
	}
	...
	if (typeof descriptorId !== "string" || descriptorId.length === 0) {
		console.warn(
			`WARN: Linux feature '${feature.id}' patch descriptor ${index + 1} must have id or name`,
		);
		return null;
	}
	...
	const wrapped = {
		...
		apply: (target, context) =>
			descriptor.apply(target, featureContext(context, feature)),
	};

	if (typeof descriptor.appliesTo === "function") {
		wrapped.appliesTo = (context) =>
			descriptor.appliesTo(featureContext(context, feature));
	}
	if (typeof descriptor.enabled === "function") {
		wrapped.enabled = (context) =>
			descriptor.enabled(featureContext(context, feature));
	}
	...
	if (typeof descriptor.targetSummary === "function") {
		wrapped.targetSummary = (context) =>
			descriptor.targetSummary(featureContext(context, feature));
	}
...
function featurePatchDescriptorListFromExports(
	feature,
	moduleExports,
	sourcePath,
	featureIndex,
) {
	const exported = moduleExports?.descriptors ?? moduleExports;
	if (exported == null) {
		console.warn(
			`WARN: Linux feature '${feature.id}' patchDescriptors entrypoint must export descriptors`,
		);
		return [];
	}
...
function normalizeEntryList(value, label, feature) {
	...
	return entries.map((entry, index) => {
		if (typeof entry === "string") {
			return {
				source: resolveFeatureRelativePath(
					feature,
					entry,
					`${label} ${index + 1}`,
				),
		...
		if (entry == null || typeof entry !== "object" || Array.isArray(entry)) {
			throw new Error(
				`Linux feature '${feature.id}' ${label} ${index + 1} must be a string or object`,
			);
		}
		...
		const name =
			entry.name == null ? path.basename(source) : String(entry.name);
		if (
			name.length === 0 ||
			path.isAbsolute(name) ||
			relativePathParts(name).includes("..") ||
			name.includes("/") ||
			name.includes("\\")
		) {
			throw new Error(
				`Linux feature '${feature.id}' ${label} ${index + 1} has invalid name`,
			);
		}
...
function normalizeInstallTarget(target, featureId) {
	return normalizeInstallRelativePath(
		target,
		`Linux feature '${featureId}' resource target`,
	);
}
...
function parseFileMode(value, fallback) {
	...
	if (typeof value !== "string") {
		throw new Error(
			`Invalid file mode: ${String(value)}; file mode must be a quoted octal string`,
		);
	}
...
function enabledLinuxFeatureInstallPlan(options = {}) {
	...
	const claimInstallTarget = (target, owner) => {
		for (const [existingTarget, existingOwner] of installTargetOwners) {
			...
			if (overlap) {
				...
				throw new Error(
					`${kind} Linux feature install target '${target}': ${owner} conflicts with '${existingTarget}' from ${existingOwner}`,
				);
			}
		}
	...
	for (const feature of loadEnabledLinuxFeatures(options)) {
		for (const [index, resource] of normalizeEntryList(
			...
			claimInstallTarget(
				target,
				`resource ${index + 1} for feature '${feature.id}'`,
			);
			resources.push({
				id: feature.id,
				...
				target,
				mode:
					resource.mode == null ? null : parseFileMode(resource.mode, 0o644),
				index,
			});
		}
		...
		if (hooks != null && (typeof hooks !== "object" || Array.isArray(hooks))) {
			throw new Error(
				`Linux feature '${feature.id}' runtimeHooks must be an object`,
			);
		}
		for (const [hookKey, hookSpec] of Object.entries(hooks ?? {})) {
			...
			if (runtimeHook == null) {
				throw new Error(
					`Linux feature '${feature.id}' has unsupported runtime hook '${hookKey}'`,
				);
			}
			for (const [index, entry] of normalizeEntryList(
				hookSpec,
				`runtimeHooks.${hookKey}`,
				feature,
			).entries()) {
				...
				claimInstallTarget(
					target,
					`runtimeHooks.${hookKey} ${index + 1} for feature '${feature.id}'`,
				);
				runtimeHooks.push({
					id: feature.id,
					key: hookKey,
					...
					mode: parseFileMode(
						entry.mode,
						runtimeHook.executable ? 0o755 : 0o644,
					),
					dir: runtimeHook.dir,
					target,
					index,
...
function stagedArtifactEntries(manifest) {
	...
	const runtimeHooks = Array.isArray(manifest.runtimeHooks)
		? manifest.runtimeHooks
		: [];
	return [...resources, ...runtimeHooks].filter(
...
function readStagedFeatureManifest(installDir) {
	...
	try {
		...
		console.warn(
			`WARN: Could not read Linux feature staged manifest at ${manifestPath}: ${error.message}`,
		);
		return null;
	}
}
...
function removeLegacyDeclarativeRuntimeHooks(installDir, options = {}) {
	const featureIds = discoverLinuxFeatureManifests(options).map(
		(feature) => feature.id,
	);
	...
	for (const runtimeHook of Object.values(RUNTIME_HOOK_DIRS)) {
		...
		if (
			!installRelativeDirectoryExists(
				installDir,
				hookDirRelative,
				"Linux feature runtime hook directory",
			)
		) {
			continue;
		}
...
function stagedLinuxFeatureFiles(appDir) {
	...
	return stagedArtifactEntries(readStagedFeatureManifest(installDir))
		.filter(
			(entry) =>
				typeof entry.target === "string" && typeof entry.mode === "string",
		)
		.map((entry) => ({
			id: entry.id ?? null,
			type: entry.type ?? null,
			key: entry.key ?? null,
			target: normalizeInstallRelativePath(
				entry.target,
				"Linux feature staged artifact target",
			),
			mode: entry.mode,
...
function stageEnabledLinuxFeatureInstall(appDir, options = {}) {
	...
	for (const resource of plan.resources) {
		copyInstallFile(
			installDir,
			resource.source,
			path.join(installDir, resource.target),
			resource.mode,
		);
		console.error(
			`Staged Linux feature resource: ${resource.id} -> ${resource.target}`,
		);
	}
	for (const hook of plan.runtimeHooks) {
		...
		console.error(
			`Staged Linux feature ${hook.key} hook: ${hook.id} -> ${path.relative(installDir, target)}`,
		);
	}
	...
	for (const feature of loadEnabledLinuxFeatures(selectedOptions)) {
		for (const [index, entry] of normalizeEntryList(
			...
			if (
				packageFormat != null &&
				formats.length > 0 &&
				!formats.includes(packageFormat)
			) {
				continue;
			}
			hooks.push({
...
function normalizePackageFormats(value, featureId, label) {
	...
	if (!Array.isArray(value)) {
		throw new Error(
			`Linux feature '${featureId}' ${label} formats must be an array`,
		);
	}
	const formats = [];
	for (const rawFormat of value) {
		const format = normalizePackageFormat(
			rawFormat,
			`package format for Linux feature '${featureId}'`,
		);
...
function normalizePackageTarget(value, featureId) {
	...
	for (const part of parts) {
		if (!PACKAGE_PATH_COMPONENT_PATTERN.test(part)) {
			throw new Error(
				`${label} contains an unsafe package path component: ${JSON.stringify(part)}`,
			);
		}
	}
	return parts.join("/");
...
function normalizePackageDependencies(feature) {
	...
	if (typeof value !== "object" || Array.isArray(value)) {
		throw new Error(
			`Linux feature '${feature.id}' packageDependencies must be an object`,
		);
	}
	...
	for (const [rawFormat, entries] of Object.entries(value)) {
		...
		if (!Array.isArray(entries)) {
			throw new Error(
				`Linux feature '${feature.id}' dependencies for ${format} must be an array`,
			);
		}
		const normalized = [];
		for (const entry of entries) {
			...
			if (
				...
				throw new Error(
					`Linux feature '${feature.id}' has invalid ${format} package dependency '${String(entry)}'`,
				);
			}
...
function enabledLinuxFeaturePackagePlan(options = {}) {
	...
	for (const feature of loadEnabledLinuxFeatures(selectedOptions)) {
		...
		if (!Array.isArray(entries)) {
			throw new Error(
				`Linux feature '${feature.id}' packageResources must be an array`,
			);
		}
		for (const [index, entry] of entries.entries()) {
			if (entry == null || typeof entry !== "object" || Array.isArray(entry)) {
				throw new Error(
					`Linux feature '${feature.id}' package resource ${index + 1} must be an object`,
				);
			}
			const source = resolveFeatureRelativePath(
				feature,
			...
			const formats = normalizePackageFormats(
				entry.formats,
				feature.id,
				`package resource ${index + 1}`,
			);
...
function enabledLinuxFeaturePackageFiles(options = {}) {
	return enabledLinuxFeaturePackagePlan(options).resources.map(
...
function assertPackageResourcesOutsideApp(packageRoot, appDir, plan) {
	...
	if (
		...
		throw new Error(
			`Packaged app directory must stay inside the package root: ${app}`,
		);
	}
	for (const resource of plan.resources) {
		if (
...
function stageEnabledLinuxFeaturePackageResources(packageRoot, options = {}) {
	...
	for (const resource of plan.resources) {
		...
		try {
			copyInstallFile(installDir, resource.source, targetPath, resource.mode, {
			...
			if (
				...
				error.message.includes("inside the install directory")
			) {
				throw new Error(
		...
		console.error(
			`Staged Linux feature package resource: ${resource.id} -> ${resource.target}`,
		);
	}
	return plan;
}

function restoreEnabledLinuxFeaturePackageResourcePermissions(
	packageRoot,
	options = {},
) {
	const root = path.resolve(packageRoot);
	...
	if (rootStat.isSymbolicLink()) {
		throw new Error(
			`Linux feature package root must not be a symbolic link: ${root}`,
		);
	}
	...
	for (const resource of plan.resources) {
		...
		if (!fs.existsSync(targetPath)) {
			throw new Error(
				`Linux feature package resource is missing from payload: ${resource.target}`,
			);
		}
		assertNoSymbolicLinkAncestors(
			root,
...
function main() {
	...
	if (command === "--package-hooks") {
		...
		if (!appDir) {
			console.error(
				"Usage: linux-features.js --package-hooks <format> <app-dir>",
			);
			process.exit(1);
		}
		for (const hook of enabledLinuxFeaturePackageHooks({
			packageFormat,
			appDir,
		})) {
			process.stdout.write(`${hook.id}\t${hook.path}\n`);
	...
	if (command === "--stage-package-resources") {
		...
		if (!packageRoot || !appDir) {
			console.error(
				"Usage: linux-features.js --stage-package-resources <format> <package-root> <app-dir>",
			);
			process.exit(1);
		}
		stageEnabledLinuxFeaturePackageResources(packageRoot, {
			packageFormat,
			appDir,
		});
		return;
	...
	if (command === "--package-dependencies") {
		...
		if (!appDir) {
			console.error(
				"Usage: linux-features.js --package-dependencies <format> <app-dir>",
			);
			process.exit(1);
		}
		for (const dependency of enabledLinuxFeaturePackageDependencies({
			packageFormat,
			appDir,
		})) {
	...
	if (command === "--package-files") {
		...
		if (!appDir) {
			console.error(
				"Usage: linux-features.js --package-files <format> <app-dir>",
			);
			process.exit(1);
		}
		for (const file of enabledLinuxFeaturePackageFiles({
			packageFormat,
			appDir,
		})) {
	...
	if (command === "--restore-package-resource-permissions") {
		...
		if (!packageRoot || !appDir) {
			console.error(
				"Usage: linux-features.js --restore-package-resource-permissions <format> <package-root> <app-dir>",
			);
			process.exit(1);
		}
		restoreEnabledLinuxFeaturePackageResourcePermissions(packageRoot, {
			packageFormat,
			appDir,
		});
		return;
	...
	if (command === "--staged-files-json") {
		...
		if (!appDir) {
			console.error(
				"Usage: linux-features.js --staged-files-json <install-dir>",
			);
			process.exit(1);
		}
		process.stdout.write(
			`${JSON.stringify(stagedLinuxFeatureFiles(appDir), null, 2)}\n`,
		);
		return;
	}
	if (command === "--enabled") {
