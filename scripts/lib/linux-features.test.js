...
test("known retired feature ids are ignored while arbitrary unknown ids fail", (t) => {
	...
	assert.deepEqual(
		linuxFeaturesConfig({ featuresRoot, featuresConfigPath: configPath }),
		{
			enabled: [],
			settings: {},
			configPath,
	...
	fs.writeFileSync(
		configPath,
		JSON.stringify({ enabled: ["open-target-discovry"] }),
	);
	assert.throws(
		() =>
			loadEnabledLinuxFeatures({
...
test("Linux feature asset matchers receive feature settings", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-asset-match-context-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	assert.equal(
		descriptor.assetMatch("current-contract", "app-current.js", {}),
		true,
	);
});
...
test("Linux feature staging rejects duplicate resource targets", (t) => {
	...
	fs.mkdirSync(path.dirname(path.join(appDir, preservedTarget)), {
		recursive: true,
	});
	...
	assert.equal(
		fs.readFileSync(path.join(appDir, preservedTarget), "utf8"),
		"preserved\n",
	);
});
...
test("Linux feature staging rejects ancestor and descendant target overlaps", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-target-overlap-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature staging rejects resource and runtime hook target collisions", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-hook-collision-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	const { featureDir, featuresRoot } = makeFeatureRoot(root, {
		id: "unsafe-link",
		title: "Unsafe Link",
		resources: [{ source: "payload.sh", target, mode: "0755" }],
		runtimeHooks: {
...
test("Linux feature staging rejects framework manifest targets", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-manifest-target-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature staging rejects normalized target aliases across features", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-cross-feature-target-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	fs.writeFileSync(
		path.join(secondFeatureDir, "feature.json"),
		`${JSON.stringify(
			{
				id: "second",
				title: "Second",
				resources: [
					{
						source: "second.txt",
						target: ".codex-linux\\features\\shared\\payload.txt",
						mode: "0644",
					},
				],
			},
...
test("Linux feature staging rejects symlinked resource sources", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-symlink-source-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature staging rejects symlinked install target parents", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-symlink-target-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	fs.mkdirSync(path.join(appDir, ".codex-linux", "features"), {
		recursive: true,
	});
	fs.mkdirSync(outside, { recursive: true });
	...
	fs.symlinkSync(
		outside,
		path.join(appDir, ".codex-linux", "features", "unsafe-link"),
		"junction",
	);
...
test("Linux feature staging rejects symlinked install target ancestors before creating parents", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-symlink-target-ancestor-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature staging does not clean stale manifest targets through symlinked parents", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-symlink-manifest-cleanup-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	assert.equal(
		fs.readFileSync(path.join(outside, "payload.txt"), "utf8"),
		"outside\n",
	);
});
...
test("Linux feature staging does not clean legacy hooks through symlinked hook dirs", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-symlink-hook-cleanup-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	fs.symlinkSync(
		outside,
		path.join(appDir, ".codex-linux", "prelaunch.d"),
		"junction",
	);
	...
	assert.equal(
		fs.readFileSync(path.join(outside, "unsafe-link-old-hook.sh"), "utf8"),
		"outside\n",
	);
});
...
test("disabled Linux features do not add package resources or dependencies", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-disabled-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("enabled Linux features stage package resources with exact modes and reject payload collisions", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-stage-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature package dependencies and files are sorted, deduplicated, and format-specific", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-lists-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	assert.deepEqual(enabledLinuxFeaturePackageDependencies(debOptions), [
		"fixture-deb-runtime",
		"zlib1g",
	]);
	...
	assert.deepEqual(
		enabledLinuxFeaturePackagePlan(debOptions).resources.map(
	...
	assert.deepEqual(enabledLinuxFeaturePackageDependencies(rpmOptions), [
		"fixture-rpm-runtime",
		"zlib",
	]);
	...
	assert.deepEqual(enabledLinuxFeaturePackageDependencies(pacmanOptions), [
		"fixture-pacman-runtime",
		"zlib",
	]);
...
test("Linux feature package resources reject traversal and package-root targets", () => {
	const cases = [
		...
		{
			target: "usr/share/codex/../../escape.txt",
			error: /must stay inside the package root/i,
		},
		...
		{ target: "DEBIAN/preinst", error: /reserved Debian control namespace/i },
		{
			target: ".PKGINFO",
			format: "pacman",
			error: /reserved pacman package namespace/i,
		},
		{
			target: ".BUILDINFO",
			format: "pacman",
		...
		},
		{
			...
			error: /unsafe package path component/i,
		},
		{
			target: "usr/%{_libdir}/bad.txt",
			error: /unsafe package path component/i,
		},
		{
			target: "usr/share/codex-package-framework/*.txt",
			error: /unsafe package path component/i,
		},
		{
			target: "usr/share/codex-package-framework/-bad.txt",
			error: /unsafe package path component/i,
		},
	];
	...
	for (const { target, format = "deb", error } of cases) {
		const root = fs.mkdtempSync(
			path.join(os.tmpdir(), "codex-feature-package-target-"),
		);
		try {
			const { featureDir, featuresRoot } = makePackageFeatureRoot(root, {
				id: "package-framework-fixture",
				title: "Package Framework Fixture",
				packageResources: [
					{ source: "payload.txt", target, mode: "0644", formats: [format] },
				],
			});
			fs.writeFileSync(path.join(featureDir, "payload.txt"), "payload\n");
...
test("Linux feature package resources reject ancestor and descendant target overlaps", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-overlap-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	const { featureDir, featuresRoot } = makePackageFeatureRoot(root, {
		id: "package-framework-fixture",
		...
		packageResources: [
			{
				source: "parent.txt",
				target: "usr/share/package-framework-fixture",
				mode: "0644",
				formats: ["deb"],
			},
			{
				source: "child.txt",
				target: "usr/share/package-framework-fixture/child.txt",
				mode: "0644",
				formats: ["deb"],
			},
		],
...
test("Linux feature package resources cannot target the packaged app directory", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-app-target-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature package resources cannot target an ancestor of the packaged app directory", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-app-ancestor-target-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	const { featureDir, featuresRoot, id } = makePackageFeatureRoot(root, {
		packageResources: [
...
test("Linux feature package resources must use regular file sources", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-source-type-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature package staging rejects symlinked resource sources", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-source-link-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature package staging rejects symlinked source ancestors", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-source-ancestor-link-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature package staging rejects symlinked target parents", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-target-link-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	...
	fs.symlinkSync(
		outside,
		path.join(packageRoot, "usr", "share", "codex-package-framework"),
		"junction",
	);
...
test("Linux feature package permission restoration rejects symlinked targets", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-restore-link-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("Linux feature package resources reject invalid modes and formats", () => {
	...
	for (const { resource, error } of invalidResources) {
		const root = fs.mkdtempSync(
			path.join(os.tmpdir(), "codex-feature-package-invalid-resource-"),
		);
		try {
			const { featureDir, featuresRoot } = makePackageFeatureRoot(root, {
				id: "package-framework-fixture",
	...
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-invalid-format-"),
	);
	try {
...
test("Linux feature package dependencies reject unsafe tokens and unsupported formats", () => {
	const invalidDependencies = [
		{
			packageDependencies: { deb: ["fixture-deb-runtime;curl"] },
			error: /invalid.*dependency/i,
		},
		{ packageDependencies: { deb: [""] }, error: /invalid.*dependency/i },
		{
			packageDependencies: { deb: "fixture-deb-runtime" },
			error: /dependencies.*array/i,
		},
		{
			packageDependencies: { appimage: ["fixture-deb-runtime"] },
			error: /unsupported package format.*appimage/i,
		},
		{
			packageDependencies: { rpm: ["runtime.so.1%(id)"] },
			error: /invalid.*dependency/i,
		},
		{
			packageDependencies: { rpm: ["runtime.so.1%{_libdir}"] },
			error: /invalid.*dependency/i,
		},
		{
	...
	for (const { packageDependencies, error } of invalidDependencies) {
		const root = fs.mkdtempSync(
			path.join(os.tmpdir(), "codex-feature-package-invalid-dependency-"),
		);
		try {
			const { featuresRoot } = makePackageFeatureRoot(root, {
				id: "package-framework-fixture",
...
test("native package plans require the app feature snapshot to match the current config", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-snapshot-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("native package plans strictly validate the current feature config", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-config-validation-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	const { featuresRoot } = makePackageFeatureRoot(root, {});
	const appDir = path.join(root, "app");
...
test("native package plans reject missing or malformed app feature snapshots", (t) => {
	const root = fs.mkdtempSync(
		path.join(os.tmpdir(), "codex-feature-package-build-info-"),
	);
	t.after(() => fs.rmSync(root, { recursive: true, force: true }));
	const { featuresRoot } = makePackageFeatureRoot(root, {});
	fs.writeFileSync(
